  ...
  renderCharacterQuest(characterId, questData, currentCharacter) {
    ...
    if (!isCompleted) {
      ...
      if (currentStep) {
        ...
        if (currentStep.checkType === 'talk_to_all' && characterId === 'isabela') {
          const talked = this.gameState.interactions ?
            Object.keys(this.gameState.interactions).filter(id =>
              this.gameState.interactions[id] > 0 && id !== characterId
            ).length : 0;
          const talkedNeeded = currentStep.checkData?.count || 6;
          stepDescription += ` (${talked}/${talkedNeeded})`;
        }

        if (currentStep.checkType === 'collect_weapons' && characterId === 'alexis') {
          const weaponsCollected = this.gameState.weaponsCollected?.alexis?.length || 0;
          const weaponsNeeded = currentStep.checkData?.count || 6;
          stepDescription += ` (${weaponsCollected}/${weaponsNeeded})`;
        }

        if (currentStep.checkType === 'all_quests_complete' && characterId === 'victor') {
          const completedQuests = Object.keys(CHARACTER_QUESTS).filter(charId => {
            if (charId === 'victor') return false;
            const progress = this.gameState.questProgress?.[charId];
            return progress?.completed === true;
          }).length;
          const questsNeeded = currentStep.checkData?.count || 7;
          stepDescription += ` (${completedQuests}/${questsNeeded})`;
        }
